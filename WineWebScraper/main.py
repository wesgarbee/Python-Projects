...
def service_function():
    ...
    # Gets the current page no. from the file to later increment
    page_number = int(address.rsplit('/', 1)[-1])

    # Makes a call to the URL in order to get the current status code
    ...
    # Runs if selection is 1. Currently the only option as Mongo is not being used at this time
    if selection == '1':
        # Initializes the count for number of wines added to the db
        added_to_SQL_db = 0
        ...
        try:
            ...
            while code is 200:
                ...
                # Gets the html from the request.get
                soup = bSoup(req.text, "html.parser")
                # Locates all of the wines in that page
